...
An R implementation of the estimator proposed by Tuğan (2021, *Econometrics Journal*) for panel vector autoregression (VAR) models with interactive fixed effects. The package jointly estimates VAR coefficients, latent common factors, and unit-specific factor loadings via an iterative algorithm that alternates between principal component estimation of the factors and least squares estimation of the VAR coefficients, following the approach of Bai (2009). Asymptotic inference is based on Theorem 2.3 of the paper.
